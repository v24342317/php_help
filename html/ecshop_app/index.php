...
function formatData($data)
{
	...
	foreach ($data['controller'] as $key => $value) {
		...
		$temp = array('type'=>$api[0], 'url'=>trim(end($api)), 'request'=>$value['request'],'__desc__'=>$value['__desc__'],'response'=>$value['response']);
		$return['api'][] = $temp;
	}
...
<body>
	...
	<div class="parms_div">
		...
		<select id="serverUrl" autocomplete="off">
			<option value="../?url=" selected>当前域名</option>
	...
	<div class="parms_div">
		<span>接口描述：</span>
				<select id="apidesc" autocomplete="off">
			<option value="" selected>接口描述</option>
				<?php foreach ($data['api'] as $key => $value): ?>
					<option value="<?php echo $value['__desc__'] ?>"  ><?php echo  $value['__desc__'] ?></option>
				<?php endforeach ?>
		</select>
	</div>
<div class="parms_div">
...
<div id="textarea" >
	...
	<span>接口返回参数：</span>
		...
		<?php foreach ($data['api'] as $key => $value): ?>
			...
			<div id='<?php echo "o".$i ?>' style="display:none;" class="parmsO">

				<form action="<?php echo $key ?>">
					<table border="1">
					<?php foreach ($value['response'] as $key => $value): ?>
						<?php 
							if (is_string($value) && strpos($value, '{') !== false && strpos($value, '}') !== false)
							{
								$letters = array('{', '}');
								...
								if (is_array($value)) {
									...
									{
										$str= trim(str_replace('!', '',$sonkey)).':<input type="text" name="'.trim(str_replace('!', '',$sonkey)).'" value="'.$sonval.'" ><br/>';
										echo $str;
									}
									echo "</td></tr>";
								}
	...
	$(function(){
		$("#apipick").change(function(){
			...
			$("#"+index).show();
			$("#o"+index).show();
			$("#apidesc").get(0).selectedIndex=index;
			<?php
			//echo 'alert("dfdfgh");'
			?>
			//$("#textarea").html($("#apidesc").find("option:selected").text()); 
		...
		$("#apidesc").change(function(){
			...
			$(".parms").hide();
			$(".parmsO").hide();
			var index = $("#apidesc :selected").index();
			$("#"+index).show();
			$("#o"+index).show();
			$("#apipick").get(0).selectedIndex=index;
